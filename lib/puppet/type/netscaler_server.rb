...
Max = 20939"
  ...
  validate do
    if self[:address] && (self[:address].match(Resolv::IPv6::Regex) || self[:address].match(Resolv::IPv4::Regex))
      if self[:translation_ip_address] || self[:translation_mask] || self[:resolve_retry] || self[:ipv6_domain]

...
Puppet::Type.newtype(:netscaler_service) do
  ...
  newproperty(:protocol) do
    ...
    validate do |value|
      if [
        ...
        raise ArgumentError, 'Valid options: HTTP, FTP, TCP, UDP, SSL, SSL_BRIDGE, SSL_TCP, DTLS, NNTP, RPCSVR, DNS, ADNS, SNMP, RTSP, DHCPRA, ANY, SIP_UDP, DNS_TCP, ADNS_TCP, MYSQL, MSSQL, ORACLE, RADIUS, RDP, DIAMETER, SSL_DIAMETER, TFTP'
      end
    end
  ...
  newproperty(:port) do
    ...
    validate do |value|
      unless value =~ %r{^\d+$} && Integer(value).between?(1, 65_535)
        raise ArgumentError, "port: #{value} is not a valid port."
      end
    end
    munge do |value|
      Integer(value)
...
Minimum value: 1"
    ...
    validate do |value|
      unless value =~ %r{^\d+$} && Integer(value).between?(1, 4_294_967_295)
        raise ArgumentError, "hash_id: #{value} must be an integer greater than 0."
      end
    end
    munge do |value|
      Integer(value)
  ...
  newproperty(:cache_type) do
    ...
    validate do |value|
      if [
        ...
        raise ArgumentError, 'Valid options: SERVER, TRANSPARENT, REVERSE, FORWARD'
      end
    end
...
Max = 31536000"
    validate do |value|
      if !value =~ %r{^\d+$}
        raise ArgumentError, "client_idle_timeout: #{value} is not a valid integer."
      end
    end
    munge do |value|
      Integer(value)
...
Max = 31536000"
    validate do |value|
      if !value =~ %r{^\d+$}
        raise ArgumentError, "server_idle_timeout: #{value} is not a valid integer."
      end
    end
    munge do |value|
      Integer(value)
...
Note: This parameter is available only when the Use Source IP (USIP) parameter is set to YES.", 'YES', 'NO')
  ...
  validate do
    if self[:clear_text_port] && ![:DTLS, :SSL, :SSL_TCP].include?(self[:protocol].to_sym)
      raise ArgumentError, 'clear_text_port may only be set for DTLS, SSL, and SSL_TCP protocols.'
    end
  end

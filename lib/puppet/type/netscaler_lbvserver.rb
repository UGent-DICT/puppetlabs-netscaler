...
Puppet::Type.newtype(:netscaler_lbvserver) do
  ...
  newproperty(:service_type) do
    ...
    validate do |value|
      if [:HTTP, :FTP, :TCP, :UDP, :SSL, :SSL_BRIDGE, :SSL_TCP, :DTLS, :NNTP, :DNS, :DHCPRA, :ANY, :SIP_UDP, :DNS_TCP, :RTSP, :PUSH, :SSL_PUSH, :RADIUS, :RDP, :MYSQL, :MSSQL, :DIAMETER, :SSL_DIAMETER, :TFTP, :ORACLE].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: HTTP, FTP, TCP, UDP, SSL, SSL_BRIDGE, SSL_TCP, DTLS, NNTP, DNS, DHCPRA, ANY, SIP_UDP, DNS_TCP, RTSP, PUSH, SSL_PUSH, RADIUS, RDP, MYSQL, MSSQL, DIAMETER, SSL_DIAMETER, TFTP, ORACLE'
      end
    end
    munge(&:upcase)
  end
  ...
  * RTSPSID - Connections that have the same RTSP Session ID belong to the same persistence session."
    ...
    validate do |value|
      if [:SOURCEIP, :COOKIEINSERT, :SSLSESSION, :RULE, :URLPASSIVE, :CUSTOMSERVERID, :DESTIP, :SRCIPDESTIP, :CALLID, :RTSPSID, :DIAMETER, :NONE].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: SOURCEIP, COOKIEINSERT, SSLSESSION, RULE, URLPASSIVE, CUSTOMSERVERID, DESTIP, SRCIPDESTIP, CALLID, RTSPSID, DIAMETER, NONE'
      end
    end
  ...
  newproperty(:persistence_backup) do
    ...
    validate do |value|
      if [:SOURCEIP, :NONE].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: SOURCEIP, NONE'
      end
    end
  ...
  * CALLIDHASH - Create a hash of the SIP Call-ID header."
    ...
    validate do |value|
      if [:ROUNDROBIN, :LEASTCONNECTION, :LEASTRESPONSETIME, :URLHASH, :DOMAINHASH, :DESTINATIONIPHASH, :SOURCEIPHASH, :SRCIPDESTIPHASH, :LEASTBANDWIDTH, :LEASTPACKETS, :TOKEN, :SRCIPSRCPORTHASH, :LRTM, :CALLIDHASH, :CUSTOMLOAD, :LEASTREQUEST].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: ROUNDROBIN, LEASTCONNECTION, LEASTRESPONSETIME, URLHASH, DOMAINHASH, DESTINATIONIPHASH, SOURCEIPHASH, SRCIPDESTIPHASH, LEASTBANDWIDTH, LEASTPACKETS, TOKEN, SRCIPSRCPORTHASH, LRTM, CALLIDHASH, CUSTOMLOAD, LEASTREQUEST'
      end
    end
  ...
  * TOS - Encode the virtual server's TOS ID in the TOS field of the IP header.
  You can use either the IPTUNNEL or the TOS option to implement Direct Server Return (DSR)."
    ...
    validate do |value|
      if [:IP, :MAC, :IPTUNNEL, :TOS].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: IP, MAC, IPTUNNEL, TOS'
      end
    end
  ...
  * DISABLED - Connection failover does not occur."
    ...
    validate do |value|
      if [:DISABLED, :STATEFUL, :STATELESS].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: DISABLED, STATEFUL, STATELESS'
      end
    end
  ...
  * OFF - Disable header insertion."
  ...
  newproperty(:mssql_server_version) do
    ...
    validate do |value|
      if [:'70', :'2000', :'2000SP1', :'2005', :'2008', :'2008R2', :'2012'].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: 70, 2000, 2000SP1, 2005, 2008, 2008R2, 2012'
      end
    end
  ...
  * If you set RHI STATE to ACTIVE on some and PASSIVE on others, the NetScaler ADC advertises the route for the VIP address if at least one of the associated virtual servers, whose RHI STATE set to ACTIVE, is in UP state."
  ...
  Possible values = PER_SECOND, PERCENT"
    ...
    validate do |value|
      if [:PER_SECOND, :PERCENT].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: PER_SECOND, PERCENT'
      end
    end
  ...
  newproperty(:skip_persistency) do
    ...
    validate do |value|
      if [:Bypass, :ReLb, :None].none? { |s| s.to_s.eql? value }
        raise ArgumentError, 'Valid options: Bypass, ReLb, None'
      end
    end
  ...
  def generate
    ...
    unless service_group_binding.nil?

...
Puppet::Type.newtype(:netscaler_rewriteglobal) do
  ...
  validate do
    if [
      ...
      raise 'Only one of invoke_policy_label or invoke_vserver_label may be specified per bind.'
    end
    ...
    if [
      ...
      raise 'connection_type cannot be set when invoking a policy label.'
    end
    ...
    if [
      ...
      raise 'When invoking a vserver, a connection type must be specified, and vice versa.'
    end
  end
end

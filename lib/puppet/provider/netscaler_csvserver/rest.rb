  ...
  def flush
    if @property_hash && !@property_hash.empty?
      ...
      if result.status == 200 || result.status == 201
        ...
        if @property_hash[:state] && (@property_hash[:state] != @original_values[:state])
        ...
        if !@original_values[:default_lbvserver] && resource[:default_lbvserver]
          binding_property_hash = { name: resource[:name], lbvserver: resource[:default_lbvserver] }
          Puppet::Provider::Netscaler.post('/config/csvserver_lbvserver_binding', message_custom(binding_property_hash, 'csvserver_lbvserver_binding'))
        ...
        elsif resource[:default_lbvserver] && resource[:default_lbvserver] != (@original_values[:default_lbvserver])

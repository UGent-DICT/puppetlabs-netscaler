...
Puppet::Type.type(:netscaler_csvserver_cachepolicy_binding).provide(:rest, parent: Puppet::Provider::NetscalerBinding) do
  ...
  def self.instances
    ...
    csvservers.each do |csvserver|
      binds = Puppet::Provider::Netscaler.call("/config/csvserver_cachepolicy_binding/#{csvserver['name']}") || []
      binds.each do |bind|
